...
public final class LitematicaSchematic extends StaticSchematic {
    ...
    private static IBlockState[] getBlockList(NBTTagList blockStatePalette) {
        IBlockState[] blockList = new IBlockState[blockStatePalette.size()];

        for (int i = 0; i < blockStatePalette.size(); i++) {
            Block block = IRegistry.BLOCK.get(new ResourceLocation((((NBTTagCompound) blockStatePalette.get(i)).getString("Name"))));
            NBTTagCompound properties = ((NBTTagCompound) blockStatePalette.get(i)).getCompound("Properties");
    ...
    private static IBlockState getBlockState(Block block, NBTTagCompound properties) {
        ...
        for (Object key : properties.keySet().toArray()) {
            IProperty<?> property = block.getStateContainer().getProperty((String) key);
    ...
    private void fillInSchematic() {
        for (String subReg : getRegions(nbt)) {
            NBTTagList usedBlockTypes = nbt.getCompound("Regions").getCompound(subReg).getList("BlockStatePalette", 10);
