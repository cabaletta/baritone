...
public class PickupCommand extends Command {
    ...
    public void execute(String label, IArgConsumer args) throws CommandException {
        Predicate<Item> filter = i -> false;
        List<Item> collecting = new ArrayList<>();
        while (args.hasAny()) {
            Item item = args.getDatatypeFor(ItemById.INSTANCE);
            filter = filter.or(item::equals);
            collecting.add(item);
        }
        if (collecting.isEmpty()) {
            baritone.getFollowProcess().follow(EntityItem.class::isInstance, 1, 0, -1);
            logDirect("Picking up all items");
        } else {
            Predicate<Item> finalFilter = filter; // hate ya, java
            baritone.getFollowProcess().follow(e -> e instanceof EntityItem && finalFilter.test(((EntityItem) e).getItem().getItem()), 1, 0, -1);
