    ...
    public PathingCommand onTick(boolean calcFailed, boolean isSafeToCancel) {
        ...
        if (calcFailed) {
            if (!knownOreLocations.isEmpty() && Baritone.settings().blacklistClosestOnFailure.value) {
                ...
                logDirect("Unable to find any path to " + mining + ", canceling Strip Mine");
                cancel();
                return null;
            }
        }
        ...
        // Fun fact that's hidden in the code: Baritone identifies ink sacs as lapis ore
    ...
    private static List<BlockPos> prune(CalculationContext ctx, List<BlockPos> locs2, List<Block> mining, int max, List<BlockPos> blacklist) {
        ...
        dropped.removeIf(drop -> {
            for (BlockPos pos : locs2) {
                if (pos.distanceSq(drop) <= 9 && mining.contains(ctx.getBlock(pos.getX(), pos.getY(), pos.getZ())) && StripMineProcess.plausibleToBreak(ctx, pos)) { // TODO maybe drop also has to be supported? no lava below?
                    return true;
                }
            }
            return false;
        ...
        List<BlockPos> locs = locs2
                ...
                .filter(pos -> StripMineProcess.plausibleToBreak(ctx, pos))
