...
public class BuildLimitPathFinder implements IElytraPathFinder {
    ...
    public CompletableFuture<UnpackedSegment> pathFindAsync(BlockPos src, BlockPos dst) {
        ...
        if(srcAboveSupportedHeight && dstAboveSupportedHeight) {
            ...
            return CompletableFuture.completedFuture(new UnpackedSegment(path.getFirst().stream(), path.getSecond()));
        }
        ...
        if(isLongDistance) {
            ...
            if(srcAboveSupportedHeight) {
                ...
                List<BetterBlockPos> path = transition.getFirst();
                boolean success = transition.getSecond();
                ...
                if(!success) {
                    ...
                    return CompletableFuture.completedFuture(new UnpackedSegment(directPath.getFirst().stream(), directPath.getSecond()));
                }
            ...
            if(dstAboveSupportedHeight) {
                ...
                if(success) {
                    ...
                    path.addAll(directPath.getFirst());
                    return CompletableFuture.completedFuture(new UnpackedSegment(path.stream(), directPath.getSecond()));
                }
