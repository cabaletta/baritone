...
public final class Settings {
    ...
    /**
     * Blocks that baritone will be allowed to break even with allowBreak set to
     * false
     */
    ...
    /**
     * Come to a halt before doing any inventory moves. Intended for anticheat such
     * as 2b2t
     */
    public final Setting<Boolean> inventoryMoveOnlyIfStationary = new Setting<>(false);

    /**
     * Disable baritone's auto-tool at runtime, but still assume that another mod
     * will provide auto tool functionality
     * <p>
     * Specifically, path calculation will still assume that an auto tool will run
     * at execution time, even though
     ...
     * <p>
     * Decrease to make Baritone more often consider paths that would require
     * placing blocks
     */
    public final Setting<Double> blockPlacementPenalty = new Setting<>(20D);

    /**
     * This is just a tiebreaker to make it less likely to break blocks if it can
     * avoid it.
     * For example, fire has a break cost of 0, this makes it nonzero, so all else
     * being equal
     * it will take an otherwise equivalent route that doesn't require it to put out
     * fire.
     */
    public final Setting<Double> blockBreakAdditionalPenalty = new Setting<>(2D);

    /**
     * Additional penalty for hitting the space bar (ascend, pillar, or parkour)
     * because it uses hunger
     */
    ...
    /**
     * Allow Baritone to fall arbitrary distances and place a water bucket beneath
     * it.
     ...
     * block.
     * This functionality is assumed to be provided by a separate library that might
     * have imported Baritone.
     * <p>
     * Note: This will prevent some usage of the frostwalker enchantment, like
     ...
     * if this is true
     * it won't sneak right click, it'll just right click, which means it'll open
     ...
     * <p>
     * Defaults to false because this fails on constantiam. Please let me know if
     * this is ever disabled. Please.
     */
    ...
    /**
     * Blocks that Baritone is allowed to place (as throwaway, for sneak bridging,
     * pillaring, etc.)
     */
     ...
     */
    public final Setting<List<Block>> blocksToAvoidBreaking = new Setting<>(new ArrayList<>(Arrays.asList( // TODO can
                                                                                                           // this be a
                                                                                                           // HashSet or
                                                                                                           // ImmutableSet?
    ...
    /**
     * this multiplies the break speed, if set above 1 it's "encourage breaking"
     * instead
     */
     ...
     * <p>
     * If a schematic asks for any block on this list at a certain position, it will
     * be treated as correct, regardless of what it currently is.
     */
     ...
     * <p>
     * If a schematic asks for a block on this mapping, all blocks on the mapped
     ...
     * <p>
     * If a schematic asks for a block on this list, only air will be accepted at
     * that location (and nothing on buildIgnoreBlocks)
     */
    ...
    /**
     * If this is true, the builder will treat all non-air blocks as correct. It
     * will only place new blocks.
     */
    public final Setting<Boolean> buildIgnoreExisting = new Setting<>(false);

    /**
     * If this is true, the builder will ignore directionality of certain blocks
    ...
    /**
     * If this setting is true, Baritone will never break a block that is adjacent
     * to an unsupported falling block.
     * <p>
    ...
    /**
     * Enables some more advanced vine features. They're honestly just gimmicks and
     * won't ever be needed in real
     * pathing scenarios. And they can cause Baritone to get trapped indefinitely in
     * a strange scenario.
     * <p>
    ...
    /**
     * Slab behavior is complicated, disable this for higher path reliability. Leave
     * enabled if you have bottom slabs
     ...
     * <p>
     * It also overshoots the landing pretty much always (making contact with the
     * next block over), so be careful
     */
    ...
    /**
     * For example, if you have Mining Fatigue or Haste, adjust the costs of
     * breaking blocks accordingly.
     */
    ...
    /**
     * If we overshoot a traverse and end up one block beyond the destination, mark
     * it as successful anyway.
     * <p>
    ...
    /**
     * How many ticks between breaking a block and starting to break the next block.
     * Default in game is 6 ticks.
     * Values under 1 will be clamped. The delay only applies to non-instant
     * (1-tick) breaks.
     */
    public final Setting<Integer> blockBreakSpeed = new Setting<>(6);

    /**
     * How many degrees to randomize the pitch and yaw every tick. Set to 0 to
     * disable
     */
     ...
     * As long as your cost heuristic is an *underestimate*, it's guaranteed to find
     * you the best path.
     * 3.5 is always an underestimate, even if you are sprinting.
     * If you're walking only (with allowSprint off) 4.6 is safe.
     * Any value below 3.5 is never worth it. It's just more computation to find the
     * same path, guaranteed.
     * (specifically, it needs to be strictly slightly less than
     * ActionCosts.WALK_ONE_BLOCK_COST, which is about 3.56)
     * <p>
     * Setting it at 3.57 or above with sprinting, or to 4.64 or above without
     * sprinting, will result in
     * faster computation, at the cost of a suboptimal path. Any value above the
     * walk / sprint cost will result
     * in it going straight at its goal, and not investigating alternatives, because
     * the combined cost / heuristic
    ...
    // a bunch of obscure internal A* settings that you probably don't want to
    // change
    /**
     * The maximum number of times it will fetch outside loaded or cached chunks
     ...
     * <p>
     * Specifically, building up the avoidance map on the main thread before pathing
     * starts actually takes a noticeable
     * amount of time, especially when there are a lot of mobs around, and your game
     * jitters for like 200ms while doing so
     */
    ...
    /**
     * When running a goto towards a container block (chest, ender chest, furnace,
     * etc),
    ...
    /**
     * When running a goto towards a nether portal block, walk all the way into the
     * portal
    ...
    /**
     * Don't repropagate cost improvements below 0.01 ticks. They're all just
    ...
    /**
     * After calculating a path (potentially through cached chunks), artificially
     * cut it off to just the part that is
     * entirely within currently loaded chunks. Improves path safety because cached
    ...
    /**
     * If a movement's cost increases by more than this amount between calculation
     ...
     * For example, if lava has spread across the path, don't walk right up to it
     * then recalculate, it might
     * still be spreading lol
     */
    public final Setting<Integer> costVerificationLookahead = new Setting<>(5);

    /**
     * Static cutoff factor. 0.9 means cut off the last 10% of all paths, regardless
     * of chunk load state
     */
    public final Setting<Double> pathCutoffFactor = new Setting<>(0.9);

    /**
     * Only apply static cutoff for paths of at least this length (in terms of
     * number of movements)
     */
    public final Setting<Integer> pathCutoffMinimumLength = new Setting<>(30);

    /**
     * Start planning the next path once the remaining movements tick estimates sum
     * up to less than this value
     */
     ...
     * 3 won't deal any damage. But if you just want to get down the mountain
     ...
     * It's not that reliable, so I've set it below what would kill an unarmored
     * player (23)
     */
     ...
     * The player overshoots the landing, but not enough to fall off. And the
     * diagonal ensures that there isn't
     * lava or anything that's !canWalkInto in that space, so it's technically safe,
     * just a little sketchy.
     * <p>
     * Note: this is *not* related to the allowDiagonalDescend setting, that is a
     * completely different thing.
     */
    public final Setting<Boolean> allowOvershootDiagonalDescend = new Setting<>(true);

    /**
     * If your goal is a GoalBlock in an unloaded chunk, assume it's far enough away
     * that the Y coord
     * doesn't matter yet, and replace it with a GoalXZ to the same place before
     * calculating a path.
     * Once a segment ends within chunk load range of the GoalBlock, it will go back
     * to normal behavior
     * of considering the Y coord. The reasoning is that if your X and Z are 10,000
     * blocks away,
     * your Y coordinate's accuracy doesn't matter at all until you get much much
     * closer.
     */
    ...
    /**
     * If a movement takes this many ticks more than its initial cost estimate,
     * cancel it
     */
     ...
     * <p>
     * If no valid path (length above the minimum) has been found, pathing continues
     * up until the failure timeout
     */
    public final Setting<Long> primaryTimeoutMS = new Setting<>(500L);

    /**
     * Pathing can never take longer than this, even if that means failing to find
     * any path at all
     */
    public final Setting<Long> failureTimeoutMS = new Setting<>(2000L);

    /**
     * Planning ahead while executing a segment ends after this amount of time, but
     * only if a path has been found
     * <p>
     * If no valid path (length above the minimum) has been found, pathing continues
     * up until the failure timeout
     */
     ...
     * even if that means failing to find any path at all
     */
    ...
    /**
     * The big one. Download all chunks in simplified 2-bit format and save them for
     * better very-long-distance pathing.
     */
    public final Setting<Boolean> chunkCaching = new Setting<>(true);

    /**
     * On save, delete from RAM any cached regions that are more than 1024 blocks
    ...
    /**
     * The chunk packer queue can never grow to larger than this, if it does, the
     * oldest chunks are discarded
     * <p>
     * The newest chunks are kept, so that if you're moving in a straight line
     * quickly then stop, your immediate render distance is still included
     */
    ...
    /**
     * Shows popup message in the upper right corner, similarly to when you make an
     * advancement
     */
    ...
    /**
     * Allow chat based control of Baritone. Most likely should be disabled when
     * Baritone is imported for use in
     * something else
     */
    public final Setting<Boolean> chatControl = new Setting<>(true);

    /**
     * Some clients like Impact try to force chatControl to off, so here's a second
     * setting to do it anyway
     */
    ...
    /**
     * Ignore depth when rendering the selection boxes (to break, to place, to walk
     * into)
     */
    ...
    /**
     * Start fading out the path at 20 movements ahead, and stop rendering it
     * entirely 30 movements ahead.
     * Improves FPS.
     */
     ...
     * {@link #smoothLookTicks} of server-sided rotations.
     */
    ...
    /**
     * When true, the player will remain with its existing look direction as often
     * as possible.
     * Although, in some cases this can get it stuck, hence this setting to disable
     * that behavior.
     */
    public final Setting<Boolean> remainWithExistingLookDirection = new Setting<>(true);

    /**
     * Will cause some minor behavioral differences to ensure that Baritone works on
     * anticheats.
     * <p>
     * At the moment this will silently set the player's rotations when using
     * freeLook so you're not sprinting in
     * directions other than forward, which is picken up by more "advanced"
    ...
    /**
     * When GetToBlockProcess or MineProcess fails to calculate a path, instead of
     * just giving up, mark the closest instance
     * of that block as "unreachable" and go towards the next closest. GetToBlock
     * expands this search to the whole "vein"; MineProcess does not.
     * This is because MineProcess finds individual impossible blocks (like one
     * block in a vein that has gravel on top then lava, so it can't break)
     * Whereas GetToBlock should blacklist the whole "vein" if it can't get to any
     * of them.
     */
     ...
     * (e.g. disconnect and reconnect, enter then exit the nether, die and respawn,
     * etc). This may literally kill your FPS and CPU because
     * every chunk gets recompiled twice as much as normal, since the cached version
     * comes into range, then the normal one comes from the server for real.
     * <p>
     * Note that flowing water is cached as AVOID, which is rendered as lava. As you
     * get closer, you may therefore see lava falls being replaced with water falls.
     * <p>
    ...
    /**
     * Always prefer silk touch tools over regular tools. This will not sacrifice
     * speed, but it will always prefer silk
     * touch tools over other tools of the same speed. This includes always choosing
     * ANY silk touch tool over your hand.
     */
    ...
    /**
     * When a new segment is calculated that doesn't overlap with the current one,
     * but simply begins where the current segment ends,
     * splice it on and make a longer combined path. If this setting is off, any
     * planned segment will not be spliced and will instead
     * be the "next path" in PathingBehavior, and will only start after this one
     * ends. Turning this off hurts planning ahead,
    ...
    /**
     * If we are more than 300 movements into the current path, discard the oldest
     * segments, as they are no longer useful
     */
    public final Setting<Integer> maxPathHistoryLength = new Setting<>(300);

    /**
     * If the current path is too long, cut off this many movements from the
     * beginning.
     */
    ...
    /**
     * After finding this many instances of the target block in the cache, it will
     * stop expanding outward the chunk search.
     */
     ...
     * if world has negative y values, subtract the min world height to get the
     * value to put here
     */
    ...
    /**
     * This will only allow baritone to mine exposed ores, can be used to stop ore
     * obfuscators on servers that use them.
     */
     ...
     * <p>
     * It is recommended to keep this value low, as it dramatically increases
     * calculation times.
     */
    public final Setting<Integer> allowOnlyExposedOresDistance = new Setting<>(1);

    /**
     * When GetToBlock or non-legit Mine doesn't know any locations for the desired
     * block, explore randomly instead of giving up.
     */
    public final Setting<Boolean> exploreForBlocks = new Setting<>(true);

    /**
     * While exploring the world, offset the closest unloaded chunk by this much in
     * both axes.
     * <p>
     * This can result in more efficient loading, if you set this to the render
     * distance.
     */
    public final Setting<Integer> worldExploringChunkOffset = new Setting<>(0);

    /**
     * Take the 10 closest chunks, even if they aren't strictly tied for distance
     * metric from origin.
     */
    ...
    /**
     * Replant nether wart while farming. This setting only has an effect when
     * replantCrops is also enabled
     */
    ...
    /**
     * When the cache scan gives less blocks than the maximum threshold (but still
     * above zero), scan the main world too.
     * <p>
     ...
     * Can help on larger builds when schematic wants to break things its already
     * built
     */
    ...
    /**
     * How far to move before repeating the build. 0 to disable repeating on a
     * certain axis, 0,0,0 to disable entirely
     */
    ...
    /**
     * Build in map art mode, which makes baritone only care about the top block in
     * each column
     */
    ...
    /**
     * Multiply the cost of placing a block that's incorrect in the builder's
     * schematic by this coefficient
     */
    public final Setting<Double> placeIncorrectBlockPenaltyMultiplier = new Setting<>(2d);

    /**
     * When this setting is true, build a schematic with the highest X coordinate
     * being the origin, instead of the lowest
     */
    public final Setting<Boolean> schematicOrientationX = new Setting<>(false);

    /**
     * When this setting is true, build a schematic with the highest Y coordinate
     * being the origin, instead of the lowest
     */
    public final Setting<Boolean> schematicOrientationY = new Setting<>(false);

    /**
     * When this setting is true, build a schematic with the highest Z coordinate
     * being the origin, instead of the lowest
     */
     ...
     * may be useful if schematics of a
     * particular format are used often, and the user does not wish to have to
     * specify the extension with every usage.
     */
    public final Setting<String> schematicFallbackExtension = new Setting<>("schematic");

    /**
     * Distance to scan every tick for updates. Expanding this beyond player reach
     * distance (i.e. setting it to 6 or above)
     * is only necessary in very large schematics where rescanning the whole thing
     * is costly.
     */
    public final Setting<Integer> builderTickScanRadius = new Setting<>(5);

    /**
     * While mining, should it also consider dropped items of the correct type as a
     * pathing destination (as well as ore blocks)?
     */
    public final Setting<Boolean> mineScanDroppedItems = new Setting<>(true);

    /**
     * While mining, wait this number of milliseconds after mining an ore to see if
    ...
    /**
     * Trim incorrect positions too far away, helps performance but hurts
     * reliability in very large schematics
     */
    public final Setting<Boolean> distanceTrim = new Setting<>(true);

    /**
     * Cancel the current path if the goal has changed, and the path originally
     * ended in the goal but doesn't anymore.
     * <p>
     * Currently only runs when either MineBehavior or FollowBehavior is active.
     * <p>
     * For example, if Baritone is doing "mine iron_ore", the instant it breaks the
     * ore (and it becomes air), that location
     * is no longer a goal. This means that if this setting is true, it will stop
     * there. If this setting were off, it would
     * continue with its path, and walk into that location. The tradeoff is if this
     * setting is true, it mines ores much faster
     * since it doesn't waste any time getting into locations that no longer contain
     * ores, but on the other hand, it misses
     * some drops, and continues on without ever picking them up.
     * <p>
     * Also on cosmic prisons this should be set to true since you don't actually
     * mine the ore it just gets replaced with stone.
     */
    public final Setting<Boolean> cancelOnGoalInvalidation = new Setting<>(true);

    /**
     * The "axis" command (aka GoalAxis) will go to a axis, or diagonal axis, at
     * this Y level.
     */
    ...
    /**
     * Disallow MineBehavior from using X-Ray to see where the ores are. Turn this
     * option on to force it to mine "legit"
     * where it will only mine an ore once it can actually see it, so it won't do or
     * know anything that a normal player
     ...
     * Basically like mining around the ores that it finds
     * in case there's one there touching it diagonally, except it checks it
     * un-legit-ly without having the mine blocks to see it.
     * You can decide whether this looks plausible or not.
     * <p>
     * This is disabled because it results in some weird behavior. For example, it
     * can """see""" the top block of a vein of iron_ore
     * through a lava lake. This isn't an issue normally since it won't consider
     * anything touching lava, so it just ignores it.
     * However, this setting expands that and allows it to see the entire vein so
     * it'll mine under the lava lake to get the iron that
     * it can reach without mining blocks adjacent to lava. This really defeats the
     * purpose of legitMine since a player could never
     ...
     * If the block above is also a goal block, set GoalBlock instead of
     * GoalTwoBlocks
     * If the block below is also a goal block, set GoalBlock to the position one
     * down instead of GoalTwoBlocks
     */
    public final Setting<Boolean> forceInternalMining = new Setting<>(true);

    /**
     * Modification to the previous setting, only has effect if forceInternalMining
     * is true
     * If true, only apply the previous setting if the block adjacent to the goal
     * isn't air.
     */
    public final Setting<Boolean> internalMiningAirException = new Setting<>(true);

    /**
     * The actual GoalNear is set this distance away from the entity you're
     * following
     * <p>
     * For example, set followOffsetDistance to 5 and followRadius to 0 to always
     * stay precisely 5 blocks north of your follow target.
     */
    public final Setting<Double> followOffsetDistance = new Setting<>(0D);

    /**
     * The actual GoalNear is set in this direction from the entity you're
     * following. This value is in degrees.
     */
    public final Setting<Float> followOffsetDirection = new Setting<>(0F);

    /**
     * The radius (for the GoalNear) of how close to your target position you
     * actually have to be
     */
    ...
    /**
     * Turn this on if your exploration filter is enormous, you don't want it to
     * check if it's done,
    ...
    /**
     * Cached chunks (regardless of if they're in RAM or saved to disk) expire and
     * are deleted after this number of seconds
     * -1 to disable
     * <p>
     * I would highly suggest leaving this setting disabled (-1).
     * <p>
     * The only valid reason I can think of enable this setting is if you are
     * extremely low on disk space and you play on multiplayer,
     * and can't take (average) 300kb saved for every 512x512 area. (note that more
     * complicated terrain is less compressible and will take more space)
     * <p>
     * However, simply discarding old chunks because they are old is inadvisable.
     * Baritone is extremely good at correcting
     * itself and its paths as it learns new information, as new chunks load. There
     * is no scenario in which having an
     * incorrect cache can cause Baritone to get stuck, take damage, or perform any
     * action it wouldn't otherwise, everything
     * is rechecked once the real chunk is in range.
     * <p>
     * Having a robust cache greatly improves long distance pathfinding, as it's
     * able to go around large scale obstacles
     * before they're in render distance. In fact, when the chunkCaching setting is
     * disabled and Baritone starts anew
     * every time, or when you enter a completely new and very complicated area, it
     * backtracks far more often because it
     * has to build up that cache from scratch. But after it's gone through an area
     * just once, the next time will have zero
    ...
    /**
     * The function that is called when Baritone will log to chat. This function can
     * be added to
     * via {@link Consumer#andThen(Consumer)} or it can completely be overriden via
     * setting
     * {@link Setting#value};
     */
    @JavaOnly
    ...
    /**
     * The function that is called when Baritone will show a toast. This function
     * can be added to
     * via {@link Consumer#andThen(Consumer)} or it can completely be overriden via
     * setting
     * {@link Setting#value};
    ...
    /**
     * The opacity of the selection. 0 is completely transparent, 1 is completely
     * opaque
     */
    ...
    /**
     * The number of ticks of elytra movement to simulate while firework boost is
     * not active. Higher values are
     * computationally more expensive.
     */
    public final Setting<Integer> elytraSimulationTicks = new Setting<>(20);

    /**
     * The maximum allowed deviation in pitch from a direct line-of-sight to the
     * flight target. Higher values are
     * computationally more expensive.
     */
    public final Setting<Integer> elytraPitchRange = new Setting<>(25);

    /**
     * The minimum speed that the player can drop to (in blocks/tick) before a
     * firework is automatically deployed.
     */
    public final Setting<Double> elytraFireworkSpeed = new Setting<>(1.2);

    /**
     * The delay after the player's position is set-back by the server that a
     * firework may be automatically deployed.
     * Value is in ticks.
     */
    public final Setting<Integer> elytraFireworkSetbackUseDelay = new Setting<>(15);

    /**
     * The minimum padding value that is added to the player's hitbox when
     * considering which point to fly to on the
     * path. High values can result in points not being considered which are
     * otherwise safe to fly to. Low values can
     * result in flight paths which are extremely tight, and there's the possibility
     * of crashing due to getting too low
     * to the ground.
     */
    ...
    /**
     * Renders the raytraces that are used in the hitbox part of the elytra fly
     * calculation.
    ...
    /**
     * Automatically path to and jump off of ledges to initiate elytra flight when
     * grounded.
     */
    public final Setting<Boolean> elytraAutoJump = new Setting<>(false);

    /**
     * The seed used to generate chunks for long distance elytra path-finding in the
     * nether.
    ...
    /**
     * Whether nether-pathfinder should generate terrain based on
     * {@link #elytraNetherSeed}.
    ...
    /**
     * Automatically swap the current elytra with a new one when the durability gets
     * too low
     */
    ...
    /**
     * Automatically land when elytra is almost out of durability, or almost out of
     * fireworks
     */
    ...
    /**
     * Maximum distance chunks can be before being culled from the nether pathfinder
     * chunk cache
     */
    ...
    /**
     * Stop all pathing & clear goal when you die
     */
    ...
    public final class Setting<T> {
        ...
        /**
         * This should always be the same as whether the setting can be parsed from or
         * serialized to a string; in other
         * words, the only way to modify it is by writing to {@link #value}
         * programatically.
         *
